...
import base64
import json
from collections import defaultdict # Used for grouping analytics
...
def escalate_email(email_id: int, request: EscalationRequest):
    ...
    try:
        if user_role == 'agent':
            ...
            db.table("emails").update({"escalated_to": 2, "is_read": True}).eq("id", email_id).execute()
            return {"msg": "Escalated to Team"}
        elif user_role == 'team_member':
            # Team member marks resolved and adds resolution details
            db.table("emails").update({"escalated_to": None, "is_read": True}).eq("id", email_id).execute()
...
# --- NEW: MARK READ ENDPOINT ---
@app.post("/mark-read/{email_id}")
def mark_email_read(email_id: int):
    ...
    try:
        # Update only the is_read status
        db.table("emails").update({"is_read": True}).eq("id", email_id).execute()
        return {"message": f"Email {email_id} marked as read."}
    except Exception as e:
        print(f"Read Status Update Error: {e}")
        raise HTTPException(status_code=500, detail="Failed to mark email read.")