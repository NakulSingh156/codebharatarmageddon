...
const INTENT_FILTERS = [
    ...
    { value: 'tech', label: 'Login/Tech' }, 
    { value: 'billing', label: 'Billing' },
];

// NEW: Status and Time Filters
...
const EmailList = ({ emails, loading, fetchEmails, filter, setFilter, selectedEmail, setSelectedEmail, getSentimentStyles, getPriorityColor, handleEmailClick }) => (
    <div className="w-4/12 bg-white border-r border-slate-200 h-screen flex flex-col fixed left-64 top-0 z-0">
        <div className="p-5 border-b border-slate-200 bg-white z-10">
            ...
            <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar space-x-2"> {/* Added space-x-2 */}
                
                {/* FILTER GROUP 1: Intent */}
                <select 
                ...
                <select 
                    onChange={(e) => setFilter(e.target.value)} 
                    value={STATUS_FILTERS.some(f => f.value === filter) ? filter : 'all'}
                    className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all border appearance-none cursor-pointer ${['unread', 'read', 'resolved'].includes(filter) ? 'bg-blue-100 text-blue-700 border-blue-300' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}
                >
                ...
                {/* FILTER GROUP 3: Time */}
                 <select 
                    onChange={(e) => setFilter(e.target.value)} 
                    value={TIME_FILTERS.some(f => f.value === filter) ? filter : 'all'}
                    className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all border appearance-none cursor-pointer ${['week', 'month'].includes(filter) ? 'bg-gray-100 text-slate-700 border-slate-300' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}
                >
                ...
                {/* Priority and Sentiment remain as simple buttons */}
                <button onClick={() => setFilter('high')} className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all border ${filter === 'high' ? 'bg-red-600 text-white border-red-600' : 'bg-white text-slate-600 border-slate-200 hover:bg-red-50'}`}>High Priority</button>
        ...
        <div className="flex-1 overflow-y-auto">
            {loading ? (
                ...
                <div className="divide-y divide-slate-100">
                    {emails.map(email => (
                        <div key={email.id} onClick={() => handleEmailClick(email)} // Updated click handler
                            className={`p-5 cursor-pointer hover:bg-blue-50 transition-all border-l-4 ${selectedEmail?.id === email.id ? 'bg-blue-50 border-blue-600 shadow-inner' : 'border-transparent'} ${email.is_read ? 'bg-white' : 'bg-blue-50/50 font-semibold text-slate-900'}`}
                        >
...
const App = () => {
  ...
  const fetchEmails = async () => {
    ...
    try {
      ...
      // We rely heavily on client-side filtering below for flexibility.
      ...
      // --- CLIENT-SIDE FILTERING (Status, Time, and Intent) ---
      ...
      const cutoffDate = (period) => {
          ...
          if (period === 'month') date.setMonth(now.getMonth() - 1);
          return date;
      };


      // 1. Time Filtering (Must be applied first to reduce the data set size before other checks)
      if (filter === 'week' || filter === 'month') {
          const cutoffTime = cutoffDate(filter);
          filteredData = filteredData.filter(email => new Date(email.received_at) >= cutoffTime);
      }


      // 2. Status Filtering (Read/Unread/Resolved)
      if (filter === 'unread') {
          filteredData = filteredData.filter(email => !email.is_read);
      } else if (filter === 'read') {
          filteredData = filteredData.filter(email => email.is_read);
      } else if (filter === 'resolved') {
          filteredData = filteredData.filter(email => email.analysis?.final_resolution_details);
      }
      ...
      // 4. Priority/Sentiment Filtering (If selected from the dropdowns/buttons)
      const isPriorityFilter = ['high', 'medium', 'low'].includes(filter);
      const isSentimentFilter = filter === 'negative';

      if (isPriorityFilter) {
          filteredData = filteredData.filter(email => email.analysis?.urgency_score === filter);
      } else if (isSentimentFilter) {
           filteredData = filteredData.filter(email => 
                email.analysis?.sentiment?.toLowerCase() === 'angry' || 
                email.analysis?.sentiment?.toLowerCase() === 'negative' ||
                email.analysis?.sentiment?.toLowerCase() === 'sad'
            );
      }
  ...
  // FIX: New handler to mark email as read when user clicks it
  const handleEmailClick = async (email) => {
    setSelectedEmail(email);
    
    // 1. Mark as read in the DB (only if current status is unread)
    if (!email.is_read) {
        try {
            await fetch(`${API_URL}/mark-read/${email.id}`, { method: 'POST' });
            
            // 2. Update local state to reflect the change immediately
            setEmails(prevEmails => 
                prevEmails.map(e => (e.id === email.id ? { ...e, is_read: true } : e))
            );
            // Update the selected email object as well
            setSelectedEmail(prevSelected => ({ ...prevSelected, is_read: true }));
        } catch (error) {
      ...
      {view === 'dashboard' ? (
        <>
            ...
            <EmailList 
                ...
                handleEmailClick={handleEmailClick} // PASS THE NEW CLICK HANDLER
            />
